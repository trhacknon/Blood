...
def welcome():
  ...
  while True:
    ...
    ##################################################################################
        ...
        printf('YOU CAN GET MORE VALID PROXY HERE: https://checkerproxy.net/archive/' + str(today))
        try:
        ...
        today = datetime.date.today()
        print(today)
        fetch = f"https://checkerproxy.net/archive/"+ str(today)
        req = requests.get("https://api.proxyscrape.com/?request=displayproxies&proxytype=http")
        array = req.text.split()
        open("proxy.txt", "w+").close()
